...
int main(int argc,char *argv[])
{
  if (argc!=5)
  {
    ...
    cout << "Example:./demo11 192.168.206.128 12345 aaa.txt 2424\n\n";
    return -1;
  }
  ...
  memset(&fileinfo,0,sizeof(fileinfo));// 清空结构体。
  //是将命令行参数中的字符串复制到结构体 fileinfo 的 filename 字段中，便于后续文件操作或记录日志时使用该文件名。
  strcpy(fileinfo.filename, argv[3]);     //将文件名拷贝到结构体中。
